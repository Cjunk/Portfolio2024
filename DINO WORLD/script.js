...
console.log(silhouettes,"HiHello World")
for (let silhouette of silhouettes) {
  silhouette.addEventListener("click", silhouetteClicked);
}
function silhouetteClicked(event) {
  /*
    Make the silhouettes & description fade out and in. 
  */
  const clickedElement = event.target
  const allDinoSils = document.getElementsByClassName('silhouette')
  const displayElem = document.getElementById('display').childNodes
  for (each of allDinoSils) {
      each.style.opacity = "20%"
  }
  clickedElement.style.opacity = "100%"
  displayElem[1].innerHTML = clickedElement.dataset['dinosaur']
  displayElem[3].src = 'images/' + dinosaurs[clickedElement.dataset['dinosaur']]['image']
  displayElem[5].style.opacity = "0%"
  displayElem[5].addEventListener('transitionend', () => {
    displayElem[5].innerHTML = dinosaurs[clickedElement.dataset['dinosaur']]['description']
    displayElem[5].style.opacity = "100%"
  })
